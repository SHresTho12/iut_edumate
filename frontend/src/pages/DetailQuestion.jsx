...
function DetailQuestion() {
  ...
  const handleUpvote = async () => {
    ...
    await axios
      .put(`/askquestion/upvote/${id}`, bodyJSON)
      .then((res) => {
        getUpdatedAnswer();
      })
  ...
  const handleDownvote = async () => {
    ...
    await axios
      .put(`/askquestion/downvote/${id}`, bodyJSON)
      .then((res) => {
        getUpdatedAnswer();
      })
  ...
  const handleanswerUpvote = async (ansid) => {
    ...
    await axios
      .put(`/answer/upvote/${ansid}`, bodyJSON)
      .then((res) => {
        getUpdatedAnswer();
      })
      .catch((err) => alert(" Already Upvoted the answer"));
  ...
  const handleanswerDownvote = async (ansid) => {
    ...
    await axios
      .put(`/answer/downvote/${ansid}`, bodyJSON)
      .then((res) => {
        getUpdatedAnswer();
      })
            ...
            <Grid templateColumns="repeat(5, 1fr)" gap={6}>
              ...
              >
                <Box p={2}>
                  ...
                  <VStack>
                    <Text fontSize="sm">
                      Posted by : {questionData?.user.displayName}
                    </Text>
                    <Text fontSize="sm">Liked by : {questionData?.upvote}</Text>
                    <Text fontSize="sm">
                      Disliked by : {questionData?.downvote}
                    </Text>
                    <HStack m={2}>
                      <Button
          ...
          <Box m={2}>
            <Box>
              ...
              <Box p={2} bg="#FAFCDB" borderRadius="10px" boxShadow={"2xl"}>
                {questionData?.answerDetails.map(
                  (_q, index) => (
                    console.log(_q),
                    (
                      <Grid templateColumns="repeat(5, 1fr)" gap={0}>
